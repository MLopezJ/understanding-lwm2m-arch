...
CoapNode.prototype.configure = function (ip, port, opts) {   // opts: { lifetime, pmax, pmin }
    ...
    var securityIid, 
        serverIid,
        shortServerId,
        serversIdInfo = this.serversIdTable['coap://' + ip + ':' + port];
    ...
    */    
    ...
    this.so.init(0, 0, {
        ...
        //0: "coap://leshan.eclipseprojects.io:5683", // LWM2M  Server URI - Leshan
    ...
    */
    ...
    this.so.init(1, 0, {
        0: 1, // Short Server ID
        1: lifetime, // Lifetime --> During "Register" or "Update" operations, the parameter Lifetime – if present – MUST match the current value of the Mandatory Lifetime Resource of the LwM2M Server Object Instance
...
CoapNode.prototype.registerAllCfg = function (callback) {
    var self = this,
    ...
    _.forEach(serverObjs, function (serverObj, iid) {
        _.forEach(securityObjs, function (securityObj, iid) {
            if (!_.isNil(serverObj.shortServerId) && !_.isNil(serverObj.shortServerId) && serverObj.shortServerId === securityObj.shortServerId && !securityObj.bootstrapServer) {
    ...
    if (requestCount === 0) {
        invokeCbNextTick(new Error('Do not have any allowed configuration.'), null, callback);
    } else {
        _.forEach(requestInfo, function (info, key) {
            ...
            self._register(serverInfo.address, serverInfo.port, info.ssid, function (err, msg) {
                ...
                if (err) {
                    ...
                    if (msg.status === RSP.created)
                        rsps.status = rsps.status || RSP.created;
                    else
                        rsps.status = msg.status;
...
CoapNode.prototype.register = function (ip, port, opts, callback) {
    ...
    var ssid = this.configure(ip, port, opts); // here is happening the Factory Bootstrap
...
CoapNode.prototype._register = function (ip, port, ssid, callback) {
    ...
    if ((!_.isString(port) && !_.isNumber(port)) || _.isNaN(port))
        throw new TypeError('port should be a string or a number.');
    console.log("payload: ", helper.checkAndBuildObjList(this, false, { ct: 'application/json', hb: true }))
    var self = this,
        reqObj = {
            hostname: ip,
            port: port,
    ...
    this._updateNetInfo(function () {
        // During "Register" or "Update" operations, the parameter Lifetime – if present – MUST match the current value of the Mandatory Lifetime Resource of the LwM2M Server Object Instance
        reqObj.query = 'ep=' + self.clientName + '&lt=' + lifetime + '&lwm2m=' + self.version ; //+ '&mac=' + self.mac;
        self.request(reqObj, agent, function (err, rsp) {
            if (err) {
                invokeCbNextTick(err, null, callback);
            } else {
                ...
                if (rsp.code === RSP.created) {
                    ...
                    // It also share with the server the IP address and port on which the client is going to be available. (these are taken directly from the source fields in IP and UDP layer headers).
        ...
        _.forEach(this.serversInfo, function (serverInfo, ssid) {
            self._update(serverInfo, updateObj, function (err, msg) {
                ...
                if (err) {
                    ...
                    if (msg.status === RSP.changed)
                        rsps.status = rsps.status || RSP.changed;
                    else
                        rsps.status = msg.status;
        ...
        _.forEach(this.serversInfo, function (serverInfo, ssid) {
            deregister(serverInfo, function (err, msg) {
                ...
                if (err) {
                    ...
                    if (msg.status === RSP.deleted)
                        rsps.status = rsps.status || RSP.deleted;
                    else
                        rsps.status = msg.status;
                    rsps.data.push({ shortServerId: ssid, status: msg.status });
                }
