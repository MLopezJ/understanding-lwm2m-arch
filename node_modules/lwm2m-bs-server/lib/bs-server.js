...
BsServer.prototype.list = function (clientNames) {
    ...
    _.map(clientNames, function (clientName) {        
        foundConfig[clientName] = self.find(clientName);
    });
...
BsServer.prototype.configure = function (clientName, configs, callback) {
    if (!_.isString(clientName))
        throw new TypeError('clientName should be a string.');

    if (!_.isPlainObject(configs) && !_.isArray(configs))
        throw new TypeError('config should be an object or an array.');
    ...
    _.forEach(configs, function (config) {
        if (!config.serverURI)
            chkErr = new Error('config should has serverURI.');
        else
            count += 1; 
    });
    ...
    if (chkErr) {
        ...
        _.forEach(configs, function (config) {
            ...
            configId = getNewConfigId(self, clientName);
            self._configBox[clientName][configId] = config;

            self._db.insert(self._configBox[clientName], function (err, newDoc) {
                if (err) {
                    chkErr = err;
                    count = 0;
                } else {
        ...
        _.forEach(configs, function (config) {
            ...
            _.forEach(self._configBox[clientName], function (obj, id) {
            ...
            self._configBox[clientName][configId] = config;
            self._db.modify(clientName, configId, config, function (err, newDoc) {
                if (err) {
                    chkErr = err;
                    count = 0;
                } else {
...
BsServer.prototype.remove = function (clientName, configId, callback) {
    var self = this,
        config = this._configBox[clientName];

    if (_.isFunction(clientName)) {
        callback = clientName;
        clientName = undefined;
    }
    
    if (_.isFunction(configId)) {
        callback = configId;
        configId = undefined;
    }
    ...
    if (clientName && config && configId) {
        ...
        this._db.removeByClientName(clientName, function (err, newDoc) {
            if (err) {
                ...
                delete self._configBox[clientName];
                invokeCbNextTick(null, clientName, callback);
            }
        });
